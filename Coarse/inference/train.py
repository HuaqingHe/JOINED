...
if __name__ == "__main__":
    ...
    for epoch in tqdm(
        ...
        for i, (img_file_name, inputs, targets1, targets2, targets3, target4) in enumerate(
            tqdm(trainLoader)
        ):
        ...
        if epoch % 1 == 0:
            if torch.cuda.device_count() > 1:
                torch.save(
                    model.module.state_dict(), os.path.join(args.save_path, str(epoch) + "val_" + str(dev_loss) + ".pt")
                    # model.state_dict(), os.path.join(args.save_path, str(epoch) + "val_" + str(dev_loss) + ".pt")
                )
            else:
                torch.save(
                    # model.module.state_dict(), os.path.join(args.save_path, str(epoch) + "val_" + str(dev_loss) + ".pt")
