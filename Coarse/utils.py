...
def build_model(model_type, encoder, pretrain, aux=False):
    ...
    if model_type == "unet++doublesmp":
        model = UNetPP_doublesmp(encoder_name=encoder,
            encoder_depth=5,
    ...
    with torch.no_grad():
        ...
        for iter, data in enumerate(tqdm(data_loader)):
            ...
            if not isinstance(outputs, list):
                outputs = [outputs]
            # print(outputs[0].size(), targets.squeeze(1).size())
            preds = torch.argmax(outputs[0].exp(), dim=1)
            ...
            dsc = 1 - dsc_loss(preds, targets.squeeze(1))
            # print(np.array(outputs[0].cpu()).shape)
            # print(np.array(targets.squeeze(1).cpu()).shape)
            losses.append(loss.item())
            dsces.append(dsc.item())
...
def generate_dataset(train_file_names, val_file_names, batch_size, val_batch_size, distance_type, do_clahe):
    ...
    # val_mean, val_std = mean_and_std(val_file_names)
    ...
    valid_loader = DataLoader(valid_dataset, batch_size=val_batch_size, num_workers=4, shuffle=True, drop_last=True)
...
def create_train_arg_parser():
    ...
    parser.add_argument("--flod", type=int, default=0, help="flod number.")
...
def generate_dataset(train_file_names, val_file_names, batch_size, val_batch_size, distance_type, do_clahe, training, aux):
    # train_mean, train_std = mean_and_std(train_file_names)
    train_mean = 0.3518534063435402
    train_std = 0.23272143499574371
    # val_mean, val_std = mean_and_std(val_file_names)

    if training:
        train_dataset = DatasetImageMaskContourDist(train_file_names, distance_type, train_mean, train_std, do_clahe, training=True, aux=aux)
        valid_dataset = DatasetImageMaskContourDist(val_file_names, distance_type, train_mean, train_std, do_clahe, training=True, aux=aux)
    else:
        train_dataset = DatasetImageMaskContourDist(train_file_names, distance_type, train_mean, train_std, do_clahe, training=True, aux=False)
        valid_dataset = DatasetImageMaskContourDist(val_file_names, distance_type, train_mean, train_std, do_clahe, training=False, aux=False)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, num_workers=8, shuffle=True, drop_last=True)
    valid_loader = DataLoader(valid_dataset, batch_size=val_batch_size, num_workers=4, shuffle=True, drop_last=True)
